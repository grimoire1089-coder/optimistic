...
func _connect_game_clock() -> void:
	...
	if clock == null:
		return
	if not clock.has_signal("season_changed"):
		return
	var callable := Callable(self, "_on_season_changed")
	if not clock.season_changed.is_connected(callable):
		clock.season_changed.connect(callable)
